...
INSTRUMENTS = (
    ...
    "TUNING_FORK",
    "KALIMBA",
    # Water is its own instrument, not a texture. It is continuous where
    # everything else here is struck, which changes how it must be scheduled
    # (a bed, not an event), how it must be released (it never decays, so a
    # short fade is a cut) and which pools may draw on it — ocean waves in a
    # bowl layer would be a category error a tag could not undo.
    "WATER",
    "TEXTURE",
    "DRONE",
    "AMBIENT",
...
FIELD_SPEC: dict[str, dict[str, tuple[str, bool]]] = {
    ...
    "levels": {
        ...
        # How loud the asset still is where playback ends, relative to its own
        # peak, measured over the last half second before `recommendedEndOffset`.
        #
        # The number the *player* needs and nothing else in the manifest
        # supplies. A bowl decays to about −63 dB by the time its tail is over,
        # so a short release is inaudible; a kalimba loop is still at −14 dB
        # because a loop does not decay, it stops — and the same short release
        # on that is the abrupt cut a listener actually hears. A runtime that
        # treats every asset the same is choosing one of those two wrongly.
        "releaseTailDb": ("number", True),
    },
    "timing": {
